...
# Açıklama
Veritabanı sistemlerinde performans, büyük ölçüde disk erişim maliyetlerinin nasıl yönetildiğine ve verinin disk üzerinde hangi yapılarla organize edildiğine bağlıdır. Disk tabanlı depolama birimleri, bellek erişimine kıyasla oldukça yavaş çalıştığından, her sorguda diske erişilmesi sistem performansını ciddi ölçüde düşürmektedir. Bu nedenle modern veritabanı yönetim sistemleri, disk erişimlerini minimize etmeye yönelik çeşitli mekanizmalar geliştirmiştir.
Bu çalışmada, PostgreSQL açık kaynak veritabanı sisteminin kaynak kodları incelenerek performansı doğrudan etkileyen iki temel yapı ele alınmıştır: Buffer Pool mekanizması ve B+ Tree veri yapısı. Buffer Pool, veritabanlarında sık kullanılan veri sayfalarının ana bellek (RAM) üzerinde tutulmasını sağlayan bir önbellekleme (caching) mekanizmasıdır. Veriler diskten satır bazında değil, sayfa (page) bazında okunur. PostgreSQL’de varsayılan sayfa boyutu 8 KB’dır.
Bir sorgu çalıştırıldığında, ilgili veri sayfasına erişim süreci ReadBuffer fonksiyonu üzerinden yönetilmektedir. ReadBuffer fonksiyonu, istenen veri sayfasının daha önce Buffer Pool içerisine alınıp alınmadığını kontrol eder. Eğer sayfa bellekte mevcutsa, disk erişimi yapılmadan doğrudan bellekten okunur. Aynı sayfaya daha sonra tekrar ihtiyaç duyulması durumunda, disk erişimi yapılmaksızın bellekteki kopyanın kullanılması sağlanır. Bu yaklaşım, disk I/O sayısını azaltarak sorgu sürelerinin kısalmasına ve sistemin genel verimliliğinin artmasına katkı sağlar.
...
Sonuç olarak Buffer Pool mekanizması ve B+ Tree veri yapısı, modern veritabanı sistemlerinde performansın temel bileşenleri arasında yer almaktadır. Buffer Pool, ReadBuffer fonksiyonu aracılığıyla sık kullanılan veri sayfalarını bellekte tutarak disk I/O maliyetini azaltırken; B+ Tree veri yapısı, verinin disk üzerinde verimli ve erişilebilir bir biçimde organize edilmesini sağlar. PostgreSQL kaynak kodları üzerinde yapılan inceleme, bu iki yapının birlikte çalışarak veritabanı sistemlerinin yüksek performanslı ve ölçeklenebilir olmasına doğrudan katkı sağladığını ortaya koymaktadır.
